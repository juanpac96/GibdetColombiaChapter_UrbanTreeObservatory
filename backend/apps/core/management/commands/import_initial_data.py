...
class Command(BaseCommand):
    ...
    def import_taxonomy(self):
        ...
        df = pd.read_csv(self.data_dir / 'taxonomy_details.csv')
        ...
        # Create genera
        genera = {}
        for _, row in tqdm(df[['family', 'genus']].drop_duplicates().iterrows(), 
    ...
    def import_functional_groups(self):
        ...
        for _, row in tqdm(df.iterrows(), desc='Creating trait values'):
            ...
            trait_values.append(TraitValue(
                trait=traits['carbon'],
        ...
        Species.objects.bulk_update(species_updates, ['functional_group'], batch_size=100)
    ...
    def import_biodiversity_records(self):
        ...
        for batch_idx in tqdm(range(total_batches), desc='Importing biodiversity records'):
        ...
        self.stdout.write(self.style.SUCCESS(f'Imported {records_created} biodiversity records'))
    ...
    def import_measurements(self):
        ...
        for chunk in tqdm(reader, desc='Importing measurements'):
            ...
            created = Measurement.objects.bulk_create(batch_measurements, batch_size=5000)
            total_measurements += len(created)
        
        self.stdout.write(self.style.SUCCESS(f'Imported {total_measurements} measurements'))

    def import_observations(self):
        self.stdout.write('Importing observations...')
        
        # Read observations_details.csv
        df = pd.read_csv(self.data_dir / 'observations_details.csv')
        ...
        for batch_idx in tqdm(range(total_batches), desc='Importing observations'):
            ...
            created = Observation.objects.bulk_create(batch_observations, batch_size=1000)
